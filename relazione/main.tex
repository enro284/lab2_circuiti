...
Come ultima misura si propone il fattore di qualità. Per un circuito RLC serie questo vale:
\begin{equation*}
	Q = \frac{1}{R_m}\sqrt{\frac{L_m}{C_m}}
\end{equation*}
Sperimentalmente può essere misurato come:
\begin{equation*}
	Q = \frac{f_r}{\Delta f}
\end{equation*}
con $\Delta f$ larghezza a metà ampiezza della funzione di risposta, o, equivalentemente,
larghezza di banda quando l'ampiezza vale $1/\sqrt{2}$ dell'ampiezza massima.
...
Dai dati sperimentali delle ampiezze, $f_{c}$ è stata misurata uguagliando numericamente le
funzioni del woofer e del tweeter. $f_{r}$ è stata calcolata come
...
\err{aggiungere correzione deriva fasi e incertezza}\\
Per quanto riguarda le fasi, il grafico (\cref{fig:phase_sweep}) si è
discostato parecchio dalla funzione attesa, specialmente sotto i $7$ kHz e 
sopra i $25$ kHz.
Data la complessità del modello non siamo riusciti a ottenere un fit convergente
neanche su regioni ristrette, di conseguenza non abbiamo potuto associare
un incertezza significativa alle misure ricavate in questo modo.
...
\begin{equation*}
	Q^{sper} = 0.88 \pm 0.12
\end{equation*}
essendo dati ricavati dal fit, quest'ultima incertezza è statistica.
...
ottenuto due stime, una teorica e una sperimentale, compatibili tra
loro.
